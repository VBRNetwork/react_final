...
const categories= [
        {name:'Writing',icon:'',url:'/writing'}, 
        {name:'Marketing & SEO',icon:'',url:'/writing'},
        {name:'Design',icon:'',url:'/writing'},
        {name:'Business Consultancy',icon:'',url:'/writing'}, 
        {name:'Developers',icon:'',url:'/writing'},
        {name:'All Categories',icon:'',url:'/writing'}
]
...
render () {
    ...
    let menuItems = categories.map((category) => {
        return  (<Menu.Item key="mail2">
                    <Link href={category.url}>
                        <a>  <Icon style={{fontSize: 17}} type={category.icon} /> {category.title}</a>
                    </Link>
                </Menu.Item>)
    });
